...
public class UIHelper {
    ...
    public static void handlerError(Context context,Throwable throwable){
        ...
        if(throwable instanceof ServerApiException){
            ...
            if(code<0){  //异常
                if(code==-1){
                    ToastUtils.showShortToast(context, R.string.http_request_timeout);
                }else{
                    ToastUtils.showShortToast(context, R.string.http_request_error);
                }
            }else{  //服务器返回错误
                String mess=exception.getMessage();
                ToastUtils.showShortToast(context,mess);
            }
        }
    }
