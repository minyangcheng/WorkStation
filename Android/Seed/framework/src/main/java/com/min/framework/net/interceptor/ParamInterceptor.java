...
public class ParamInterceptor implements Interceptor {
    ...
    private Request addMobileInfoToReqHeader(Request request) {
        ...
        try {
            Head head = new Head();
            head.apiVersion = ConfigConstants.VERSION_NAME;
            head.mobileVersion = ConfigConstants.SDK_INT;
            head.channel = ConfigConstants.FLAVOR;
    ...
    public void handleParams(Map<String, String> params) {
        params.put("source", ConfigConstants.SOURCE);
        String mapUrl = mapToString(params);
        params.put("signature", EncryptUtils.encryptMD5ToString(mapUrl + EncryptUtils.encryptMD5ToString(ConfigConstants.APP_SECRET)));
    }
