...
export default function jsbridgeMixin(hybrid) {
   ...
   * @param handler
   */
  hybridJs.on = function on(page, type, handler) {
    messageHandlers[page+"_"+type] = handler;
  };
   ...
   * @param handler
   */
  hybridJs.off = function off(page, type) {
    delete messageHandlers[page+"_"+type]
  };
    ...
    function doDispatchMessageFromNative() {
      ...
      if (responseId) {
        ...
        keyArr.forEach(function (value, index) {
          if(value.indexOf(message.handlerName, value.length - message.handlerName.length) !== -1){
